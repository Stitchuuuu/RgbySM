...
void getCharactersUsedIn(ArrayUTF32 *arr, wchar_t *chars) {

	int numChars = wcslen(chars);
	for (int i = 0; i < arr->length; i++) {
		// printf("Line %u / %u...\n", i, arr->length);
		wchar_t* line = arr->items[i];
	...
	chars[numChars] = 0;
	printf("char array finished ! Used chars: %u\n", numChars);
}


void getCharactersUsedInApp(wchar_t *chars) {
	getCharactersUsedIn(&i18n_AppTexts, chars);
}
